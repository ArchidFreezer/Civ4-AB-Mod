...
class CvDotMapOverlayScreen:
	...
	def initVars(self):
		...
		"""
		global g_DotMap
		g_DotMap = StrategyOverlay.getDotMap()
		for index in range(len(StrategyOverlay.COLOR_KEYS)):
	...
	def createColorPanel(self):
		...
		"""
		screen = self.getScreen()
		count = len(StrategyOverlay.COLOR_KEYS)
		width = StrategyOverlay.PALETTE_WIDTH
		...
		for index, color in enumerate(StrategyOverlay.COLOR_KEYS):
			row = index / width
			column = index % width
	...
	def selectColor(self, index):
		...
		"""
		self.currentColor = gc.getInfoTypeForString(StrategyOverlay.COLOR_KEYS[index])  # index
