...
PALETTE_WIDTH = 3
COLOR_KEYS = ('COLOR_PLAYER_BROWN', 'COLOR_PLAYER_PEACH', 'COLOR_PLAYER_PEACH_TEXT', 'COLOR_PLAYER_PALE_RED', 'COLOR_RED', 'COLOR_PLAYER_PINK', 'COLOR_PLAYER_PALE_ORANGE', 'COLOR_PLAYER_ORANGE', 'COLOR_PLAYER_DARK_YELLOW', 'COLOR_GREAT_PEOPLE_RATE', 'COLOR_YELLOW', 'COLOR_PLAYER_LIGHT_YELLOW', 'COLOR_PLAYER_DARK_GREEN', 'COLOR_PLAYER_GREEN', 'COLOR_PLAYER_LIGHT_GREEN', 'COLOR_PLAYER_DARK_CYAN', 'COLOR_PLAYER_MIDDLE_CYAN', 'COLOR_PLAYER_CYAN', 'COLOR_PLAYER_DARK_BLUE', 'COLOR_PLAYER_BLUE', 'COLOR_PLAYER_LIGHT_BLUE', 'COLOR_PLAYER_DARK_PURPLE', 'COLOR_MAGENTA', 'COLOR_PLAYER_PURPLE_TEXT', 'COLOR_BLACK', 'COLOR_LIGHT_GREY', 'COLOR_WHITE')

gc = CyGlobalContext()
options = BugCore.game.StrategyOverlay

g_layers = {}

MSG_ADD_CITY = CvUtil.getNewEventID("StrategyOverlay.AddCity")
MSG_REMOVE_CITY = CvUtil.getNewEventID("StrategyOverlay.RemoveCity")

class StrategyOverlayEventHandler:

	def __init__(self, customEM ) :

		print "Initializing Strategy Overlay Mod"

		self.customEM = customEM

		# Loading and Saving
		self.customEM.addEventHandler( 'GameStart', self.onGameStart )
		self.customEM.addEventHandler( 'OnLoad', self.onLoad )
		self.customEM.addEventHandler( "OnPreSave", self.onPreSave )
		self.customEM.addEventHandler( 'PythonReloaded', self.onLoad )
		# Hiding/Showing During HotSeat Games
		self.customEM.addEventHandler( 'BeginActivePlayerTurn', self.onBeginActivePlayerTurn )
		self.customEM.addEventHandler( 'SwitchHotSeatPlayer', self.onSwitchHotSeatPlayer )
		# Interaction
...
def getDotMap():
	global g_DotMap
	if g_DotMap is None:
		BugUtil.error("StrategyOverlay has not been initialized")
	return g_DotMap

def hideDotMap(args=None):
	getDotMap().hide()
	options.setShowDotMap(False)

def toggleDotMapVisibility(args=None):
	getDotMap().toggleVisibility()
	options.setShowDotMap(getDotMap().visible)

def toggleDotMapEditMode(args=None):
	getDotMap().toggleEditing()
	if not getDotMap().editing and not options.isShowDotMap():
		getDotMap().hide()
...
class DotMapLayer(StrategyLayer):
	...
	def __init__(self):
		...
		self.MAX_DOT_STYLE = PlotStyles.PLOT_STYLE_CIRCLES
		self.BFC_OFFSETS = []
		for x in range(-2, 3):
			for y in range(-2, 3):
	...
	def updateData(self, data):
		...
		for player in ArchidUtils.players(alive=True, human=True):
	...
	def addCityAt (self, point, color, layer):
		...
		"""
		CyMessageControl().sendModNetMessage(MSG_ADD_CITY, ArchidUtils.getActivePlayerID(), point[X] * 1000 + point[Y], color, layer)
	...
	def addCity(self, ePlayer, city):
		...
		self.dirty = True
		if ePlayer == ArchidUtils.getActivePlayerID():
	...
	def removeCityAt(self, point):
		...
		"""
		ePlayer = ArchidUtils.getActivePlayerID()
	...
	def drawCities(self, skip=None):
		...
		for city in self.iterCities(ArchidUtils.getActivePlayerID()):
	...
	def readOptions(self):
		self.CROSS_ALPHA = self.percentToAlpha(options.getDotMapBrightness())
		self.DOT_ALPHA = self.percentToAlpha(options.getDotMapBrightness())
		self.HIGHLIGHT_CROSS_ALPHA = self.percentToAlpha(options.getDotMapHighlightBrightness())
		self.HIGHLIGHT_DOT_ALPHA = self.percentToAlpha(options.getDotMapHighlightBrightness())
		self.DRAW_DOTS = options.isDotMapDrawDots()
		self.DOT_STYLE = min(self.MAX_DOT_STYLE, max(0, options.getDotMapDotIcon()))
