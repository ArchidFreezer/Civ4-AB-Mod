...
class BugOptionsTab:
	...
	def createMainPanel (self, screen, error = False):
		...
		if error:
			...
			exitCallback = "handleExitButtonInput"
			# Reload button
			title = BugUtil.getPlainText("TXT_KEY_ARCHID_OPT_RELOAD", "Reload")
		...
		hover = BugUtil.getPlainText("TXT_KEY_ARCHID_OPT_EXIT_HOVER", "Exits the A-Mod Options screen.")
	...
	def addCheckbox (self, screen, panel, name, spacer=False):
		...
		if (option is not None):
			...
			if spacer:
				...
				self.addSpacer(screen, hbox, hbox, CHECKBOX_INDENT)
				panel = hbox
			screen.attachCheckBox(panel, control, option.getTitle(), self.callbackIFace, "handleArchidCheckboxClicked", name, value)
	...
	def addTextEdit (self, screen, labelPanel, controlPanel, name):
		...
		if (option is not None):
			...
			screen.attachEdit(controlPanel, control, value, self.callbackIFace, "handleArchidTextEditChange", name)
			screen.setToolTip(control, option.getTooltip())
	...
	def addTextDropdown (self, screen, labelPanel, controlPanel, name, spacer=False, layout="left"):
		...
		if (option is not None):
			...
			return self.addDropdown(screen, labelPanel, controlPanel, name, spacer, layout, elements, index, "handleArchidDropdownChange")
		else:
			self.addMissingOption(screen, controlPanel, name)
	...
	def addIntDropdown (self, screen, labelPanel, controlPanel, name, spacer=False, layout="right"):
		...
		if (option is not None):
			index = option.getIndex()
			elements = tuple(option.getDisplayValues())
			return self.addDropdown(screen, labelPanel, controlPanel, name, spacer, layout, elements, index, "handleArchidIntDropdownChange")
		else:
			self.addMissingOption(screen, controlPanel, name)
	...
	def addFloatDropdown (self, screen, labelPanel, controlPanel, name, spacer=False, layout="right"):
		...
		if (option is not None):
			index = option.getIndex()
			elements = tuple(option.getDisplayValues())
			return self.addDropdown(screen, labelPanel, controlPanel, name, spacer, layout, elements, index, "handleArchidFloatDropdownChange")
		else:
			self.addMissingOption(screen, controlPanel, name)
	...
	def addCheckboxTextDropdown (self, screen, checkPanel, dropPanel, checkName, dropName, layout="left", spacer=False):
		...
		if (checkOption is not None and dropOption is not None):
			...
			checkControl, dropControl = self.addCheckboxDropdown(screen, checkPanel, dropPanel, checkName, dropName, layout, elements, index, "handleArchidDropdownChange", spacer)
			return checkControl, dropControl
		else:
			if (checkOption is None):
				self.addMissingOption(screen, checkPanel, checkOption)
	...
	def addCheckboxIntDropdown (self, screen, checkPanel, dropPanel, checkName, dropName, layout="right", spacer=False):
		...
		if (checkOption is not None and dropOption is not None):
			index = dropOption.getIndex()
			elements = tuple(dropOption.getDisplayValues())
			checkControl, dropControl = self.addCheckboxDropdown(screen, checkPanel, dropPanel, checkName, dropName, layout, elements, index, "handleArchidIntDropdownChange", spacer)
			return checkControl, dropControl
		else:
			if (checkOption is None):
				self.addMissingOption(screen, checkPanel, checkOption)
	...
	def addCheckboxFloatDropdown (self, screen, checkPanel, dropPanel, checkName, dropName, layout="right", spacer=False):
		...
		if (checkOption is not None and dropOption is not None):
			index = dropOption.getIndex()
			elements = tuple(dropOption.getDisplayValues())
			checkControl, dropControl = self.addCheckboxDropdown(screen, checkPanel, dropPanel, checkName, dropName, layout, elements, index, "handleArchidFloatDropdownChange", spacer)
			return checkControl, dropControl
		else:
			if (checkOption is None):
				self.addMissingOption(screen, checkPanel, checkOption)
	...
	def addSlider (self, screen, labelPanel, controlPanel, name, spacer=False, vertical=False, expanding=True, fill=None, min=0, max=100):
		...
		if (option is not None):
			...
			value = option.getRealValue()
			if vertical:
				screen.attachVSlider(controlPanel, control, self.callbackIFace, "handleArchidSliderChanged", name, min, max, value)
				if expanding:
					screen.setLayoutFlag(control, "LAYOUT_SIZE_VEXPANDING")
			else:
				screen.attachHSlider(controlPanel, control, self.callbackIFace, "handleArchidSliderChanged", name, min, max, value)
				if expanding:
