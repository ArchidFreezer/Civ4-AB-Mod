...
class IniFile(object):
	...
	def fillComments(self):
		...
		self.config.addInitialComment(BugUtil.getPlainText("TXT_KEY_ARCHID_CREATED_BY_HEADER"))
		self.config.addInitialComment()
		defaultHeader = BugUtil.getPlainText("TXT_KEY_ARCHID_DEFAULT") + ": "
		for option in self.options:
			if not option.isParameterized():
	...
	def setValue(self, section, key, value):
		...
		if value is not None and self.isLoaded():
			...
			if old is None or old != value:
				sect[key] = value
				gc.clearIniOptMaps() # Tell the dll that its cache is invalid
				self.dirty = True
...
class BaseOption(AbstractOption):
	...
	def __init__(self, mod, id, type, default=None, andId=None, dll=None, 
		...
		self.xmlKey = "TXT_KEY_ARCHID_OPT_" + self.id.upper()
		self.title = title
		...
		if isinstance(values, str):
			if self.isStringList():
				if values.find("|") != -1:
					...
					self.values = []
					self.valuesXmlKey = "TXT_KEY_ARCHID_OPT_" + values.upper()
			elif self.isIntList():
